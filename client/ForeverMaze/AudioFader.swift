...
public class AudioFader: NSObject {
  ...
  func fade(fromVolume fromVolume: Double, toVolume: Double,
    ...
    velocity: Double = iiFaderForAvAudioPlayer_defaultVelocity, onFinished: ((Bool)->())? = nil) {
      ...
      self.fromVolume = AudioFader.makeSureValueIsBetween0and1(fromVolume)
      self.toVolume = AudioFader.makeSureValueIsBetween0and1(toVolume)
  ...
  func timerFired(timer: NSTimer) {
    ...
    let currentTimeFrom0To1 = AudioFader.timeFrom0To1(
    ...
    if fadeIn {
      volumeMultiplier = AudioFader.fadeInVolumeMultiplier(currentTimeFrom0To1,
        velocity: fadeVelocity)
      
      newVolume = fromVolume + (toVolume - fromVolume) * volumeMultiplier
      
    } else {
      volumeMultiplier = AudioFader.fadeOutVolumeMultiplier(currentTimeFrom0To1,
        velocity: fadeVelocity)
