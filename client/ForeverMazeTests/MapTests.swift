...
class MapTests : XCTestCase {
  ...
  static let bottomLeftPos = MapPosition(x: 0, y: 0)
  static let topRightPos = MapPosition(x: (Config.worldSize.width - 1), y: (Config.worldSize.width - 1))
  static let centerPos = MapPosition(x: Config.worldSize.width/2, y: Config.worldSize.height/2)
  ...
  func testMapBoxWrapping() {
    ...
    let xWrapPos = MapPosition(x: Config.worldSize.width - Config.screenTiles.width + 1, y: MapTests.centerPos.y)
    ...
    let xNoWrapPos = MapPosition(x: Config.worldSize.width - Config.screenTiles.width, y: MapTests.centerPos.y)
    let xNoWrap = MapBox(origin: xNoWrapPos, size: Config.screenTiles)
    XCTAssert(xNoWrap.wraps == false, "\(xNoWrap) should NOT wrap")
  }
  
  func testMapPositionIndicies() {
  ...
  func testMapBoxContains() {
    ...
    for box in boxes {
      ...
      let basePos = MapPosition(xIndex: box.origin.xIndex - 1, yIndex: box.origin.yIndex - 1)
      for var i=0; i<=Int(box.size.width+1); i++ {
        for var j=0; j<=Int(box.size.height+1); j++ {
          let pos = MapPosition(xIndex: basePos.xIndex + i, yIndex: basePos.yIndex + j)
